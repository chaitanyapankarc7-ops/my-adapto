...
public class VoiceAssitentPage extends AppCompatActivity {
    ...
    private void startPulseAnimation() {
        ...
        AlphaAnimation alpha = new AlphaAnimation(0.2f, 0.5f);
        alpha.setDuration(1500);
        alpha.setRepeatCount(Animation.INFINITE);
        alpha.setRepeatMode(Animation.REVERSE);
        
        viewGlow.startAnimation(scale);
        viewGlow.setAlpha(0.3f); // Base alpha
    }
    ...
    private void speakAndAppend(String text) {
        if (txtStatus != null) {
            txtStatus.setText("Tap to Speak");
            txtStatus.setTextColor(android.graphics.Color.parseColor("#A0AEC0"));
        }
